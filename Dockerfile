...
COPY apps/mcp-vrm/package.json apps/mcp-vrm/package.json
...
RUN pnpm --filter @kajidog/vrm-mcp build
...
RUN pnpm install --prod --frozen-lockfile --ignore-scripts --filter @kajidog/vrm-mcp...

COPY --from=build /app/apps/mcp-vrm/dist apps/mcp-vrm/dist
...
CMD ["node", "apps/mcp-vrm/dist/index.js"]
